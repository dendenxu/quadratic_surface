...
Camera::Camera(int width, int height, float fx, float fy)
    : width(width),
      ...
      drag_state(std::make_unique<DragState>()) {
...
void Camera::update(bool transform_from_vecs, bool copy_cuda) {
    if (transform_from_vecs && !lock_trans) {
        ...
        float dot = glm::dot(glm::cross(v_world_up, v_back), drag_state->drag_start_right);
...
void Camera::begin_drag(float x, float y, bool is_panning, bool about_origin) {
    ...
    drag_state->drag_start = glm::vec2(x, y);
    drag_state->drag_start_back = v_back;
    drag_state->drag_start_right = v_right;
    drag_state->drag_start_up = v_up;
    drag_state->drag_start_center = center;
    drag_state->drag_start_origin = origin;
    drag_state->is_panning = is_panning;
    drag_state->about_origin = about_origin;
}
void Camera::drag_update(float x, float y) {
    if (!drag_state->is_dragging) return;
    glm::vec2 drag_curr(x, y);
    glm::vec2 delta = drag_curr - drag_state->drag_start;
    delta *= -2.f * movement_speed / std::max(width, height);
    if (drag_state->is_panning) {
        center = drag_state->drag_start_center +
                 delta.x * drag_state->drag_start_right -
                 delta.y * drag_state->drag_start_up;
        if (drag_state->about_origin) {
            origin = drag_state->drag_start_origin +
                     delta.x * drag_state->drag_start_right -
                     delta.y * drag_state->drag_start_up;
        }
    } else {
        ...
        glm::vec3 v_back_new = m * glm::vec4(drag_state->drag_start_back, 1.f);

        v_back = glm::normalize(v_back_new);

        if (drag_state->about_origin) {
            center = glm::vec3(m * glm::vec4(drag_state->drag_start_center - origin, 1.f)) + origin;
        }
        update(true, false);
    }
}
bool Camera::is_dragging() const { return drag_state->is_dragging; }
void Camera::end_drag() { drag_state->is_dragging = false; }
void Camera::move(const glm::vec3& xyz) {
    center += xyz * movement_speed;
    if (drag_state->is_dragging) {
        drag_state->drag_start_center += xyz * movement_speed;
    }
}
