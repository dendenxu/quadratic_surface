...
void add_common_opts(cxxopts::Options& options) {
    ...
    options.add_options()
        ...
        ("w, width", "image width", cxxopts::value<int>()->default_value("960"))
        ("h, height", "image height", cxxopts::value<int>()->default_value("540"))
        ("fx", "focal length in x direction; -1 = 1111 or default for NDC",
            cxxopts::value<float>()->default_value("-1.0"))
        ("fy", "focal length in y direction; -1 = use fx",
            cxxopts::value<float>()->default_value("-1.0"))
        ("bg", "background brightness 0-1", cxxopts::value<float>()->default_value("0"))
