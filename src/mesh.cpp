...
uniform mat4x4 MV;
uniform mat4x4 M;
uniform mat4x4 V; // this is currently useless due to MV
...
out highp vec4 WorldFragPos;
out highp vec4 CamFragPos;
out highp vec3 Normal;

void main()
{
    vec4 pos = vec4(aPos, 1.0); // in object space
    WorldFragPos = M * pos; // now FragPos is in world
    // ! will this be faster if computed in frag shader?
    CamFragPos = V * WorldFragPos; // camera space coords
    Normal = normalize(mat3x3(M) * aNormal);

    gl_Position = K * CamFragPos; // doing a perspective projection to clip space
    VertColor = aColor;
}
)glsl";
...
in vec4 WorldFragPos; // Note: this is in world space
in vec4 CamFragPos; // Note: this is in camera space
in vec3 Normal; // Note: this is in world space
...
void main()
{
    ...
    if (useFaceNormal) {
        vec3 xTangent = dFdx(vec3(WorldFragPos));
        vec3 yTangent = dFdy(vec3(WorldFragPos));
    ...
    if (unlit) {
        ...
        float diffuse = diffuseStrength * max(dot(lightDir, shadeNormal), 0.0);
        float diffuse2 = diffuse2Strength * max(dot(lightDir2, shadeNormal), 0.0);

        vec3 viewDir = normalize(camPos - vec3(WorldFragPos)); // ! is this really ok? FragPos was in world coords
...
Mesh::Mesh(int n_verts, int n_faces, int face_size, bool unlit)
    ...
    if (program == -1) {
        ...
        u_V = glGetUniformLocation(program, "V");
        ...
        u_use_face_normal = glGetUniformLocation(program, "useFaceNormal");
    }
}
...
void Mesh::draw(const glm::mat4x4& V, glm::mat4x4 K) const {
    ...
    glUniformMatrix4fv(u_MV, 1, GL_FALSE, glm::value_ptr(MV));         // o2c
    glUniformMatrix4fv(u_M, 1, GL_FALSE, glm::value_ptr(transform_));  // o2w
    glUniformMatrix4fv(u_V, 1, GL_FALSE, glm::value_ptr(V));           // w2c
    glUniformMatrix4fv(u_K, 1, GL_FALSE, glm::value_ptr(K));           // c2clip
    glUniform3fv(u_cam_pos, 1, glm::value_ptr(cam_pos));
    glUniform1i(u_unlit, true);
    glUniform1i(u_use_face_normal, true);
    ...
    if (faces.empty()) {
        ...
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo_);
        glDrawElements(get_gl_ele_type(face_size), faces.size(), GL_UNSIGNED_INT, (void*)0);
    }
    glBindVertexArray(0);
}
